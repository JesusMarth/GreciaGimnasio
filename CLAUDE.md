...
`socios` (incl. `dni` opcional, `estado` activo|baja, `fecha_baja` desde v1.3: se apunta al dar de baja y se limpia al reactivar; bajas antiguas sin fecha) · `suscripciones` (1 socio→N; actividad, importe, periodicidad mensual|bono, `pagado_hasta`, `cobertura_manual`, `activa`; desde v1.8 `sesiones_por_bono` y `sesiones_manual` para **bonos por sesiones**; desde v1.10 `meses` = duración de la cuota por tiempo: 1 mensual · 3 · 6 · 12 anual, lo que cubre cada cobro por defecto) · `pagos` (cabecera) · `pago_lineas` (N por pago; copia `actividad` para informes; `sesiones` desde v1.8: sesiones compradas por la línea, NULL en mensuales) · `asistencias` (desde v1.8: una fila por sesión picada de un bono; `ON DELETE CASCADE`) · `eventos` (desde v1.5: historial de movimientos por socio — `registrarEvento()` en `server/eventos.ts`, nunca rompe la operación principal; al estrenar la tabla se reconstruye de los datos existentes con `reconstruirEventos()` en `db.ts`; `socio_nombre` copiado y `ON DELETE SET NULL` para que sobreviva al borrado del socio) · `tarifas` (plantillas de precio) · `config` (clave/valor: `email.*`, `datos.*`).

## Lógica de dominio
El **estado de cuota se CALCULA** (no se guarda) desde `pagado_hasta` vs hoy (`util.ts` → `estadoDe`): pendiente / atrasado / pronto (≤5 días, `UMBRAL_PRONTO`) / aldia. El `estadoResumen` del socio = el peor de sus subs activas. Un cobro puede pagar varias actividades; cada línea avanza `pagado_hasta` (solo adelanta, nunca retrocede). Filosofía: **agnóstica a ofertas** — se guarda el importe real cobrado; la app no calcula descuentos.
**Cuotas con duración (v1.10)**: `periodicidad='mensual'` + `meses` (1/3/6/12). Un anual de 324 € es una cuota por tiempo con `meses=12`: el importe es por periodo y un cobro sin `meses` explícitos avanza `pagado_hasta` esos meses. En cada cobro meses e importe son libres (no hace falta tarifa). Los `periodicidad='bono'` con `sesiones_por_bono NULL` (anteriores a v1.8) son **ambiguos**: la ficha obliga a reclasificarlos como cuota por tiempo (conserva fecha y cobros) o como bono de sesiones.
**Bonos por sesiones (v1.8)**: una suscripción con `periodicidad='bono'` **y** `sesiones_por_bono > 0` (`esBonoPorSesiones`) no usa fecha: su estado sale de `estadoBono(restantes)` con restantes = SUM(`pago_lineas.sesiones`) + `sesiones_manual` − COUNT(`asistencias`), siempre calculado (`sesionesDe` en `queries.ts`; umbral «quedan pocas» `UMBRAL_SESIONES_PRONTO = 3`; puede quedar negativo = a deber). Reutiliza los 4 estados de cuota, así que Panel/filtros/Métricas no distinguen. `pagadoHasta` se devuelve `null` para esos bonos (en BD se conserva, nunca se toca). **Bonos anteriores a v1.8** (`sesiones_por_bono NULL`) siguen por fecha tal cual (`bonoSinConfigurar` → aviso); al configurarlos, sus líneas con `sesiones NULL` cuentan un bono completo cada una; una cuota mensual que se convierte a bono marca sus líneas a `sesiones = 0`. Picar = POST `/suscripciones/:id/asistencias` (la UI SIEMPRE confirma antes y ofrece Deshacer = DELETE `/asistencias/:id`).
